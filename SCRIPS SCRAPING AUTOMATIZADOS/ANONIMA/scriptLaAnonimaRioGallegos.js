...
async function scrapeData() {
  ...
  try {
    console.log(" ");
    console.log(chalk.green.underline('                  [LA ANONIMA RIO GALLEGOS]                         '));
    console.log(" ");
           ...
           // console.log("SELECTOR "+seencontroSelector);
            if(intentos < 2){
                try{
                    ...
                    const opcionProvincia = '5';
                    await page.select(selectProvincia, opcionProvincia);

                    const selectLocalidad = '#sel_localidad_5'; 
                    const opcionLocalidad = '14';
...
async function createExcel(rows) {
  ...
  const fileName= 'LaAnonima_RioGallegos'+fechaConSeparador+'.xlsx';
