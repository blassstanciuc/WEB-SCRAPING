...
let opcionASeleccionar = prompt("Ingrese el numero correspondiente a la sucursal a scrapear: 1) HIPER LA PLATA | 2) SAN MARTIN'");
if(opcionASeleccionar==1){
    opcionASeleccionar="La Plata";
}else{
    opcionASeleccionar="san martin";
}

// Preguntar al usuario que ingrese los enlaces separados por comas
const enlaces = prompt('Por favor, ingrese los enlaces separados por comas: ');
...
try {
    console.log(" ");
    console.log(chalk.green.underline('                  [SUCURSAL CARREFOUR '+opcionASeleccionar+' X CATEGORIA]                         '));
    console.log(" ");
      ...
      try{
        ...
        while (!selectorFound && attempts < maxAttempts) {
          try {
            ...
            await page.waitForSelector('section.vtex-product-summary-2-x-container', { timeout: 5000 });
        ...
        if (selectorFound) {
          ...
          const isOnePage = await page.evaluate(() => {
            let elemento = document.querySelectorAll('div.valtech-carrefourar-search-result-2-x-paginationButtonPages');
            ...
            await openProducts(enlacesProductos,page,arrayProductos,rows);//OBTENGO LOS PRODUCTOS DE LA PAGINA INICIAL
            selectorFound = false;
            attempts = 0;
  ...
  //--------------------------------------------------- Esperar y Recorrer pagina 
    ...
    try {
        const Productos = await page.evaluate(() => {
            const links = document.querySelectorAll('section.vtex-product-summary-2-x-container--contentProduct');
            //console.log(links);
            let productos = [];
      ...
      console.log("error al obtener info de productos");
      return false; 
    }
  //-----------------------------------------------------------------------------  
...
async function openProducts(productos,page,arrayProductos,rows) {
    ...
    for (const producto of allProductos) {
      try {
        const resultado = await page.evaluate((producto) => {
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = producto;
          // obtengo la fecha-----------------------------------------------------------------------------------------
          ...
          //OBTENGO LOS DATOS------------------------------------------------------------------------------------------
          const marcaProducto = tempDiv.querySelector('span.vtex-store-components-3-x-productBrandName')?.innerText;
          const tdElementEan = tempDiv.querySelector('td[data-specification="EAN"]');
          let eanElement;
          if (tdElementEan) {
          ...
          const descripcion = tempDiv.querySelector('span.vtex-product-summary-2-x-brandName')?.innerText.trim() || "";
          const priceProducto = tempDiv.querySelector('span.valtech-carrefourar-product-price-0-x-sellingPriceValue span')?.innerText.trim();
          let precioAntiguo = tempDiv.querySelector('span.valtech-carrefourar-product-price-0-x-listPriceValue.strike span')?.innerText;
          const promoGeneral = tempDiv.querySelector('span.valtech-carrefourar-product-price-0-x-sellingPriceValue span.valtech-carrefourar-product-price-0-x-discountPercentage')?.innerText;
          let whatPromoGeneral = promoGeneral? promoGeneral.split(" "):"";
          console.log(whatPromoGeneral);
          const promoDescripcion = tempDiv.querySelector('span.tooltipText')?.innerText;
          let whatPromo = promoDescripcion? promoDescripcion.split(" "):"";
          console.log(whatPromo);
          let precioUnitarioPromocional = tempDiv.querySelector('#productInfoContainer div.product_discount div.first_price_discount_container span.price_discount')?.innerText;
          const precioXunidadMedida = tempDiv.querySelector('span.valtech-carrefourar-dynamic-weight-price-0-x-currencyContainer')?.innerText;
          console.log(precioXunidadMedida)
          const unidad = tempDiv.querySelector('span.valtech-carrefourar-dynamic-weight-price-0-x-unit')?.innerText.split(" ");
          //---------------------------------------------------------------------------------------------------------------
        ...
        console.log("TERMINE DE GUARDAR INFO DEL PRODUCTO",resultado);
  ...
  async function openNextPage(page,browser,arrayProductos,rows,totalPage,x,link){
      ...
      try{
          if(nextPagina !== false ){ //Mientras que haya una siguiente pagina 
              ...
              await openProducts(nuevosProductos,page,arrayProductos,rows); //Obtenemos la info de los productos
    ...
    let intentos = ix;
      try {
        ...
        console.log("SELECTOR "+seencontroSelector);
          ...
          await page.keyboard.type(opcionASeleccionar);
          await page.waitForTimeout(5000);
          await page.keyboard.press('Enter');
          //-------------------------------
          await page.waitForTimeout(20000);
...
async function createExcel(rows) {
  ...
  const fileName= 'Carrefour_'+opcionASeleccionar+"_MANUAL_"+fechaConSeparador+'.xlsx';
