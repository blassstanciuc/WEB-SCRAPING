...
async function scrapeData() {
  ...
  // EJEMPLO DE ESCRAPEO
  // "https://www.carrefour.com.ar/Desayuno-y-merienda/Galletitas-bizcochitos-y-tostadas?initialMap=c,c&initialQuery=desayuno-y-merienda/galletitas-bizcochitos-y-tostadas&map=category-1,category-2,brand&query=/desayuno-y-merienda/galletitas-bizcochitos-y-tostadas/oreo&searchState"
  ]

  try {
    console.log(" ");
    console.log(chalk.green.underline('                  [SUCURSAL CARREFOUR SAN MARTIN X CATEGORIA]                         '));
    console.log(" ");
    ...
    for (const producto of allProductos) {
      try {
        const resultado = await page.evaluate((producto) => {
          ...
          //SUCURSAL
          arrayProductos.push("San Martin");
...
async function createExcel(rows) {
  ...
  const fileName= 'Carrefour_San_Martin_x_categoria'+fechaConSeparador+'.xlsx';
