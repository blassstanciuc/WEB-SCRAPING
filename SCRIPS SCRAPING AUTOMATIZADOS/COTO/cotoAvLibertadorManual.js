  ...
  try {
    ...
    try{
        if(i==1){
          ...
          await page.waitForSelector('tbody tr.mute td.mute_address_td');
          const btnSucursal = await page.$('tbody tr.mute td.mute_address_td');
          //const btnSucursalTortugas = await page.$('tbody tr.even td.select_address_td');

          if (btnSucursal){
            await btnSucursal.click();
    ...
    console.log(" ");
    console.log(chalk.green.underline('                         [SUCURSAL COTO AV LIBERTADOR]                         '));
    console.log(" ");
  ...
  for (const enlace of productos) {
    ...
    try {
      ...
      const resultado = await nuevaPagina.evaluate(() => {
        ...
        //SUCURSAL
        arrayProductos.push("AV Libertador");
        //FECHA
