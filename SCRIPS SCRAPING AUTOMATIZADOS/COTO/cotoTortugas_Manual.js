...
const arrayProductos = [];
  try {
    ...
    if(i==1){
      ...
      if(selectorImputAdress){
        ...
        if (btnLogin) {
          ...
          await page.click('#myModal > div > span');
          await page.evaluate(async (waitTime) => {
            // Utiliza la función setTimeout para esperar el tiempo especificado
            await new Promise(resolve => setTimeout(resolve, waitTime));
        }, 5000);
        await page.click('tbody tr.mute td.mute_address_td');
          await page.waitForNavigation();
          await page.evaluate(async (waitTime) => {
              // Utiliza la función setTimeout para esperar el tiempo especificado
              await new Promise(resolve => setTimeout(resolve, waitTime));
    ...
    console.log(" ");
    console.log(chalk.green.underline('                         [SUCURSAL COTO TORTUGAS MANUAL]                         '));
    console.log(" ");
...
async function createExcel(rows) {
  ...
  const fileName= 'Coto_Tortugas_manual'+fechaConSeparador+'.xlsx';
