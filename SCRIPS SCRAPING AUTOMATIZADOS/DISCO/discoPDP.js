...
async function scrapeData() {
  ...
  try {
    console.log(" ");
    console.log(chalk.red.underline('                         [SUCURSAL DISCO]                         '));
    console.log(" ");
    ...
    for(let link of linksCategorias){
      ...
      try{
            ...
            if(i==1){
              ...
              const selectorImput = await page.$('div.discoargentina-delivery-modal-1-x-deliveryInput label div input');
              if(selectorImput){
              ...
              await page.click('body > div.vtex-modal-layout-0-x-modal.vtex-modal-layout-0-x-modal--store-selector > div > div > div.discoargentina-delivery-modal-1-x-container.flex.flex-column.ph6.pv7 > div.discoargentina-delivery-modal-1-x-clientInfoContainer.flex.flex-column.justify-center.w-90.center > div.flex.flex-row.justify-center.pt5 > div > button > div');
              await page.evaluate(async (waitTime) => {
                  // Utiliza la función setTimeout para esperar el tiempo especificado
                  await new Promise(resolve => setTimeout(resolve, waitTime));
              }, tiempoDeEsperaEnMilisegundos);
              ...
              const SelectorSelectRegion = 'div.discoargentina-delivery-modal-1-x-dropdownStoreForm div label div.vtex-styleguide-9-x-container select'; 
              ...
              const SelectorSelectTienda = 'body > div.vtex-modal-layout-0-x-modal.vtex-modal-layout-0-x-modal--store-selector > div > div > div.discoargentina-delivery-modal-1-x-container.flex.flex-column.ph6.pv7 > div.undefined.flex.flex-column > div.flex.flex-row.justify-around-l.pv2 > div.w-50.pl5.discoargentina-delivery-modal-1-x-dropdownStoreForm > div > label > div > select'; 
              ...
              await Promise.all([
                ...
                page.click('body > div.vtex-modal-layout-0-x-modal.vtex-modal-layout-0-x-modal--store-selector > div > div > div.discoargentina-delivery-modal-1-x-container.flex.flex-column.ph6.pv7 > div.undefined.flex.flex-column > div.discoargentina-delivery-modal-1-x-buttonsContainer.flex.flex-row.justify-around.pv1 > div.discoargentina-delivery-modal-1-x-buttonStyle.pl5-m.w-100.flex.flex-row.justify-center > button')
              ]);
      ...
      for (const producto of linksProductos) {
        try{
          ...
          const resultado = await pageProducto.evaluate(() => {
            ...
            let promoMenosPorcentaje = document.querySelector('div.discoargentina-store-theme-SpFtPOZlANEkxX04GqL31 span')?.innerText;
            let promoEj2X1 = document.querySelector('div.discoargentina-store-theme-Aq2AAEuiQuapu8IqwN0Aj span')?.innerText;
            let promoSegundoAlPorcentaje = document.querySelector('div.discoargentina-store-theme-1LCA-xHQ8NgNHQ062m5gTL span.discoargentina-store-theme-MnHW0PCgcT3ih2-RUT-t_')?.innerText;
            ...
            //PRECIO REGULAR
            let precioAntiguo = document.querySelector('div.discoargentina-store-theme-2t-mVsKNpKjmCAEM_AMCQH')?.innerText;
            arrayProductos.push(precioAntiguo==null || precioAntiguo==undefined?document.querySelector('div.discoargentina-store-theme-1oaMy8g_TkKDcWOQsx5V2i div.discoargentina-store-theme-1dCOMij_MzTzZOCohX1K7w')?.innerText:document.querySelector('div.discoargentina-store-theme-2t-mVsKNpKjmCAEM_AMCQH')?.innerText);
            //PRECIO PROMOCIONAL
              if(promoEj2X1){
              ...
              let unitarioPromocional = document.querySelector('div.discoargentina-store-theme-1oaMy8g_TkKDcWOQsx5V2i div.discoargentina-store-theme-1dCOMij_MzTzZOCohX1K7w')?.innerText;
              ...
              //PRECIO OFERTA
              arrayProductos.push(promoMenosPorcentaje!=null || promoMenosPorcentaje!=undefined?document.querySelector('div.discoargentina-store-theme-1oaMy8g_TkKDcWOQsx5V2i div.discoargentina-store-theme-1dCOMij_MzTzZOCohX1K7w')?.innerText:"");
...
async function createExcel(rows) {
  ...
  const fileName= 'Disco_CityBell_'+fechaConSeparador+'.xlsx';
