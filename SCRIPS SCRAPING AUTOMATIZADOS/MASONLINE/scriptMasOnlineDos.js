...
async function cargarPaginaConReintentos(url,nuevaPagina, page) {
  for (let i = 0; i < 3; i++) {
    try {
  ...
  console.error(`No se pudo cargar la página después de varios intentos.`);
  await nuevaPagina.close();
  return false;
}


async function openProducts(productos,browser,rows,page) {
  // Recorre los enlaces y obtén datos de cada página
  console.log(chalk.red("                   OBTENIENDO DATOS..."));
  let nuevaPagina = await browser.newPage();
    ...
    try { 
      for (const enlace of productos) {
        ...
        const paginaCargada = await cargarPaginaConReintentos(enlace, nuevaPagina,page);
        if (!paginaCargada) {
     ...
     await nuevaPagina.close();
      ...
      //await nuevaPagina.close();
      console.log("FINALIZANDO CATEGORIA");
    } 
