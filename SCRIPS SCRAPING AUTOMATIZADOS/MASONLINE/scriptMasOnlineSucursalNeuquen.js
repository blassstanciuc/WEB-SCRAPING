...
async function scrapeData() {
  ...
  //    "https://www.masonline.com.ar/kiosco?initialMap=c&initialQuery=kiosco&map=category-1,category-2&query=/kiosco/chocolates&searchState",
  //   "https://www.masonline.com.ar/kiosco?initialMap=c&initialQuery=kiosco&map=category-1,category-2&query=/kiosco/golosinas&searchState",
  //   "https://www.masonline.com.ar/kiosco?initialMap=c&initialQuery=kiosco&map=category-1,category-2&query=/kiosco/alfajores&searchState",
  ...
  //  "https://www.masonline.com.ar/lacteos?initialMap=c&initialQuery=lacteos&map=category-1,category-2,category-2,category-2,category-2&query=/lacteos/cremas/dulce-de-leche/mantecas-y-margarinas/postres-y-flanes-frescos&searchState",
  //  "https://www.masonline.com.ar/lacteos?initialMap=c&initialQuery=lacteos&map=category-1,category-2&query=/lacteos/leches&searchState",
  //  "https://www.masonline.com.ar/lacteos?initialMap=c&initialQuery=lacteos&map=category-1,category-2&query=/lacteos/yogures&searchState",
  ...
  //  "https://www.masonline.com.ar/fernet-y-aperitivos/fernet",
  //  "https://www.masonline.com.ar/aceites-vinagres-y-aderezos/aderezos",
  //  "https://www.masonline.com.ar/caldos-sopas-y-pure/pure",
  //  "https://www.masonline.com.ar/pa%C3%B1ales-e-higiene/pa%C3%B1ales",
  //  "https://www.masonline.com.ar/ropa/jabones"
  // prueba 
  "https://www.masonline.com.ar/gaseosas/cola"
    ]
  ...
  try {
    console.log(" ");
    console.log(chalk.green.underline('                         [SUCURSAL MAS ONLINE NEUQUEN]                         '));
    console.log(" ");
...
async function loadMoreProducts(page, browser, rows) {
  ...
  // const cantProductos = await page.evaluate(()=>{
  //   let productos = document.querySelector("div.vtex-flex-layout-0-x-flexCol--productCountCol div.vtex-flex-layout-0-x-flexColChild--productCountCol div.valtech-gdn-search-result-0-x-totalProducts--layout span")?.innerText.split(" ");
  //   return productos[0];
  // })
  // let cantidadDeProductos = Number(cantProductos);
  // console.log("CANTIDAD DE PRODCUTOS "+cantProductos);
  ...
  const btnMoreProducts = await page.$("div.valtech-gdn-search-result-0-x-buttonShowMore--layout div.valtech-gdn-search-result-0-x-buttonShowMore a div");// CAMBIE
  console.log("EL BOTON ES "+btnMoreProducts);
  await page.waitForTimeout(5000);
  if (btnMoreProducts!=undefined ||btnMoreProducts!=null ) {
      console.log("HAY BOTON");
      await page.click("div.valtech-gdn-search-result-0-x-buttonShowMore--layout div.valtech-gdn-search-result-0-x-buttonShowMore a div");// CAMBIE
      //await page.waitForNavigation();
      await page.waitForTimeout(5000);
      await loadMoreProducts(page, browser, rows);
          ...
          const htmlProductos = await page.evaluate(() => {
            const itemsProductos = document.querySelectorAll('div.valtech-gdn-search-result-0-x-galleryItem');
            ...
            // if (htmlProductos.length < cantidadDeProductos){
            //   console.log(" Cantidad de productos: "+"["+htmlProductos.length+"]");
      ...
      for (const producto of allProductos) {  
        try {
          const resultado = await page.evaluate((Productos) => {
            ...
            const descripcion = tempDiv.querySelector('div.vtex-product-summary-2-x-nameContainer h3.vtex-product-summary-2-x-productNameContainer span.vtex-product-summary-2-x-productBrand')?.innerText;
  
            const priceProducto = tempDiv.querySelector('div.valtech-gdn-dynamic-product-1-x-weighablePriceWrapper div.valtech-gdn-dynamic-product-1-x-dynamicProductPrice span')?.innerText;
            const precioAntiguo = tempDiv.querySelector('div.valtech-gdn-dynamic-product-1-x-weighablePriceWrapper div.valtech-gdn-dynamic-product-1-x-weighableSavings span')?.innerText;
            const precioUnidadDeMedida = tempDiv.querySelector('div.vtex-flex-layout-0-x-flexRowContent--price-per-kg div div div div span.valtech-gdn-dynamic-weight-price-0-x-currencyContainer')?.innerText;
            const UnidadDeMedida = tempDiv.querySelector('div.vtex-flex-layout-0-x-flexRowContent--price-per-kg div div div div span.valtech-gdn-dynamic-weight-price-0-x-unit')?.innerText;
            const promoConPorcentaje = tempDiv.querySelector('div.valtech-gdn-dynamic-product-1-x-dynamicProductPrice div.valtech-gdn-dynamic-product-1-x-weighableSavingsContainer span.valtech-gdn-dynamic-product-1-x-weighableSavingsPercentage')?.innerText;
            // const promoNueva = tempDiv.querySelector('div.valtech-gdn-custom-highlights-0-x-customHighlightImageContainer div.valtech-gdn-custom-highlights-0-x-customHighlightImageContainerIndividual img');
            // const altPromocionSinPorcentaje = promoNueva?promoNueva['alt']:"";
            // const promocionSinPorcentaje = altPromocionSinPorcentaje.split("-") 
            const promo2do = tempDiv.querySelector('div.valtech-gdn-dynamic-product-1-x-dynamicProductPrice div.valtech-gdn-dynamic-product-1-x-weighableSavingsContainer span.valtech-gdn-dynamic-product-1-x-promoVolumenTxtTwo')?.innerText;
            // console.log(altPromo);

            //CADENA
            arrayProductos.push("Masonline");
            //SUCURSAL
            arrayProductos.push("Neuquen");
            //FECHA
            ...
            //PRECIO PROMOCIONAL
            // if(promoConPorcentaje || promocionSinPorcentaje){
            //     if (promocionSinPorcentaje[0].trim() === "Banco" || promocionSinPorcentaje[0].trim() === "Volumen") {
            //       arrayProductos.push(promocionSinPorcentaje[1].trim());
            //   } else {
            //       arrayProductos.push(promoConPorcentaje ? promoConPorcentaje : promocionSinPorcentaje[0].trim());
            //   }
            // }
            if(promoConPorcentaje || promo2do){
                  arrayProductos.push(promoConPorcentaje ? promoConPorcentaje : promo2do);
            }else{
              arrayProductos.push("")
            }
...
async function selectSucursal(page){
  try{
      ...
      await page.waitForSelector('body > div.render-container.render-route-store-home > div > div.vtex-store__template.bg-base > div > div.vtex-store-header-2-x-headerStickyRow.vtex-store-header-2-x-headerStickyRow--mobile-wrapper-main.sticky.z-999 > div.vtex-store-header-2-x-headerRow.vtex-store-header-2-x-headerRow--mobile-wrapper-main > div > section > div > div.valtech-gdn-region-login-0-x-buttonWrapper.flex.flex-column.items-center > button > div');
      ...
      if(selectorImputAdress && selectorImputPass){
          ...
          const btnConfirmar = await page.$("div.valtech-gdn-region-login-0-x-continueButton button");
          await btnConfirmar.click();
          await page.waitForTimeout(3000);

          const checkSucursal = await page.$("div.valtech-gdn-region-login-0-x-circle div");
          await checkSucursal.click();
          await page.waitForTimeout(3000);

          const btnContinuar = await page.$("div.valtech-gdn-region-login-0-x-continueButton button");
...
async function createExcel(rows) {
  ...
  const fileName= 'masOnlineNeuquen_'+fechaConSeparador+'.xlsx';
