//SE HACE UN BUCLE INFINITO EN EL PASO DE PAGINAS
...
async function scrapeData() {
  ...
  const linksCategorias = [ 
    "http://ecommerce.nini.com.ar:8081/ventas.online/?nini.controllers.login"
  ]

  try {
    console.log(" ");
    console.log(chalk.green.underline('                  [NINI]                         '));
    console.log(" ");
    for(let link of linksCategorias){
      ...
      try{
        ...
        if (selectorFound) {
          ...
          if(i==1){
            await page.goto("http://ecommerce.nini.com.ar:8081/ventas.online/?nini.controllers.login");
      ...
      try {
        const resultado = await page.evaluate((Productos) => {
         ...
         console.log(priceProducto);
          ...
          //SUCURSAL
          arrayProductos.push("Rio Gallegos");
          //FECHA
          ...
          arrayProductos.push(promoGeneral?priceProducto:"");
          //PRECIO OFERTA
          arrayProductos.push(promocionOff?priceProducto:"");
    ...
    try{
        if(nextPage){ //Mientras que haya una siguiente pagina 
          console.log("ingrese a la pag siguiente")
            await page.click('form[name="_paginacion"] div div:nth-last-child(2)'); //CLICK EN EL BTN PARA QUE PASE AL SIGUIENTE
            console.log("----------------------------------------");
        ...
        while (!seencontroSelector && intentos < maxIntentos) {
           ...
           // console.log("SELECTOR "+seencontroSelector);
            if(intentos < 2){
                try{
                    //INICIO LOGIN
                    const selectorImputUser = await page.$('#userName');
                    if(selectorImputUser){
                        await selectorImputUser.type('30125');
                        const selectorImputPassword = await page.$('#password');
                        await selectorImputPassword.type('reset');
                        await page.click('#login');
                        await page.waitForTimeout(2000); 
                    }else{
                        console.log("ERROR CON EL LOGIN DEL SITIO");
                    }
                }catch{
                    console.error('error al setear valores de ingreso');
                }
                try{
                    const btnCrearPedido = await page.$("#crearPedido");
                    await btnCrearPedido.click();
                    await page.waitForTimeout(2000); 
                }catch{
                  console.error('error al clickear btn de crear pedido');
                }
                try{
                    const btnContinuar = await page.$("#next");
                ...
                try{
                    const btnGoToHome = await page.$("#goToHome");
                    await btnGoToHome.click();
                    await page.waitForTimeout(2000); 
                }catch{
                  console.error('error al clickear btn de crear pedido');
                }
...
async function createExcel(rows) {
  ...
  const fileName= 'Nini'+fechaConSeparador+'.xlsx';
  ...
  //const filePath = 'C:/Users/Administrator/Desktop/WEB_SCRAPING/ARCHIVOS/CARREFOUR/SAN_MARTIN/' + fileName; // Cambia la ruta según tus necesidades
